...
{- [Question 2.2]:
   ...
   It is not possible to extend out implementation, with this reduction 
   rule, without introducing non-determinism. 

   Take the following example:

        loop ((λx. x) y) ((λx. x) y) (e3)
    
    There are two possible rules that can be used to reduce this expression:
    ...
    or the new rule 
        ...
        e2 -> e2' / loop e1 e2 e3 -> loop e1 e2' e3

        loop ((λx. x) y) ((λx. x) y) (e3)
        -> loop ((λx. x) y) (y) (e3)
    
    There is nothing to tell us, or the program, which rule to use first. 
    So, a non-deterministic choice must be made. Therefore, we cannot extend 
    our implementation with this rule. 
